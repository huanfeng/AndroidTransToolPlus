      ...
      <el-scrollbar height="200px" ref="scrollRef" class="log-scroll">
        <div class="logs">
          <div v-for="log in logStore.filteredLogs" :key="log.id" class="log-line">
            <el-tag size="small" :type="tagType(log.level)" style="width:74px; text-align:center">{{ levelName(log.level) }}</el-tag>
...
// 自动滚动到底部
watch(() => logStore.filteredLogs.length, async () => {
  await nextTick()
  try {
...
// 面板打开时也滚动到底部
watch(() => configStore.config.showLogView, async (v) => {
  if (v) {
    await nextTick()
    try {
      const api = scrollRef.value as any
...
<style scoped>
.log-layout { display: flex; align-items: stretch; }
.log-scroll { flex: 1; }
.side-tools { width: 120px; border-left: 1px solid var(--el-border-color); padding: 8px; display: flex; flex-direction: column; gap: 8px; }
.w-full { width: 100%; }
.full-btn { width: 100%; display: inline-flex; justify-content: center; }
/* 去除 Element Plus 纵向堆叠按钮默认的水平间距，避免不对齐 */
.side-tools :deep(.el-button + .el-button) { margin-left: 0 !important; }
.logs { font-family: ui-monospace, SFMono-Regular, Menlo, Consolas, monospace; font-size: 12px; padding: 8px; padding-bottom: 32px; }
.log-line { display: grid; grid-template-columns: 80px 80px 1fr; gap: 8px; align-items: center; padding: 2px 0; }
.time { color: var(--ep-text-color-placeholder); }
